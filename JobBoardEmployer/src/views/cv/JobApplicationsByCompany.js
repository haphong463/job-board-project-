...
const JobApplicationsByCompany = () => {
  ...
  useEffect(() => {
    ...
    // Giải mã token để lấy companyId
    try {
      const decodedToken = jwtDecode(token);
      const companyId = decodedToken.company; // Giả sử companyId có trong token

      axios.get(`http://localhost:8080/api/job-applications/company/${companyId}`, {
        headers: {
  ...
  // Tính toán các ứng dụng hiện tại
  ...
  const handleApprove = (applicationId) => {
    ...
    axios.put(`http://localhost:8080/api/job-applications/approve/${applicationId}`, {}, {
      headers: {
        Authorization: `Bearer ${token}`
      }
    })
      .then(response => {
        // Cập nhật trạng thái của ứng dụng công việc
        setJobApplications(prevState =>
          prevState.map(application =>
            application.id === applicationId ? { ...application, approved: true } : application // Assuming false means approved
          )
        );
  ...
  const downloadPdf = async (applicationId) => {
    try {
      const token = localStorage.getItem('accessToken');
      const response = await axios.get(`http://localhost:8080/api/application/download-pdf/${applicationId}`, {
        headers: {
          Authorization: `Bearer ${token}`
        },
          ...
          <CCard>
            <CCardBody>
                          ...
                          <CCardBody>
                            ...
                            {isMissingCVDetails ? (
                              <CAlert color="warning">
                                CV details are not available. Please <Link to="/buy">purchase a full CV package</Link> to view full details.
                              </CAlert>
                            ) : (
                              <>
              ...
              <div className="pagination">
                {Array.from({ length: Math.ceil(jobApplications.length / applicationsPerPage) }, (_, index) => (
                  <button
                    key={index}
