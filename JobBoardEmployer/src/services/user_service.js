...
export const getAllUserAsync = async (query, role, page, size) =>
  await axiosRequest.get(`${URL}/search?query=${query}&role=${role}&page=${page}&size=${size}`)
export const getUserByIDAsync = async (id) => await axiosRequest.get(`${URL}/${id}`)
...
export const deleteUserAsync = async (id) => await axiosRequest.delete(`${URL}/${id}`)
...
export const getAllPermission = async () => await axiosRequest.get('/user/permissions')
