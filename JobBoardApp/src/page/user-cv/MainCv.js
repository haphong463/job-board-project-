...
function MainCv() {
  ...
  const handleAnswerSubmit = (userAnswer) => {
    if (userAnswer === deleteQuestion.answer) {
      ...
      if (newWrongAttempts >= 3) {
        setNotification({
          type: "error",
          message: "Too many incorrect attempts. Delete operation cancelled.",
        });
        ...
        setNotification({
          type: "error",
          message: `Incorrect answer. Try again. (Attempt ${newWrongAttempts}/3)`,
        });
  ...
  const confirmDelete = async () => {
    try {
      ...
      setNotification({
        type: "error",
        message: "Failed to delete CVs. Please try again.",
      });
    }
    setDeleteQuestion(null);
    setShowQuestionForm(false);
          ...
          switch (currentAction) {
            case "create":
              return hasCVs ? (
                ...
                >
                  You already have a CV. Please go to the{" "}
                  <Link to="/list-template">
                    <i className="render-css"> list template page</i>
                  </Link>{" "}
                  or{" "}
                  <i
                    className="render-css"
                  ...
                  {hasCVs ? (
                    <>
                      ...
                      {!showQuestionForm ? (
                        <>
                          <p>
                            Are you sure you want to delete all your CV's
                            information?
                          </p>
                          <button className="confirm-btn" onClick={deleteCv}>
                            Confirm Delete
                          </button>
                        </>
                      ) : (
                        <div className="question-form">
                          <p className="text-danger">
                            *Answer the following question to confirm delete:
                          </p>
                    ...
                    >
                      You don't have any CVs to delete!
                    </h3>
                  )}
                </div>
              );
